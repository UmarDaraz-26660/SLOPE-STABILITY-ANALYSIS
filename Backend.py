...
MODEL_PATH = os.path.join(BASE_DIR, "slope_model.pkl")
...
# ============================================================
# LOAD MODEL (NO TRAINING ON SERVER)
# ============================================================
def load_model():
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError("❌ slope_model.pkl not found")
...
def predict_fos(user_input):
    ...
    for ui_key, model_key in COLUMN_MAP.items():
        if ui_key not in user_input:
            raise KeyError(f"Missing input: {ui_key}")
        mapped_input[model_key] = float(user_input[ui_key])
...
def sensitivity_analysis(user_input, variable, num_points=50):
    ...
    if variable not in ranges:            
        raise ValueError("Invalid sensitivity variable")
    ...
    for val in x_vals:
        ...
        temp[variable] = float(val)
        y_vals.append(predict_fos(temp))
